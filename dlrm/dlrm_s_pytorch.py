...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln):
        # m = m_spa, args.arch_sparse_feature_size = 16 given
        emb_l = nn.ModuleList()
        print("Create_emb", self.qr_flag)
        for i in range(0, ln.size):
            n = ln[i] # 100, 50
            print(n)
            # construct embedding operator
            if self.qr_flag and n > self.qr_threshold:
                print("Create QREmbedding")
                EE = QREmbeddingBag(n, m, self.qr_collisions,
                    operation=self.qr_operation, mode="sum", sparse=True)
                # num of category = 100, 50
                # embedding_dim = number of sparse feature = 16
                # 5 (optional) => collison number (hash?)
                # index of category x = 0~99, 0~49
                # quotient = 0~99 \ 5
                # remainder = 0~99 mod 5 (Hash)
            elif self.md_flag:
                base = max(m)
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        # print(self.emb_l)
        # (0): QREmbeddingBag([25, 4], [16, 16], mode=sum)
        # (1): QREmbeddingBag([13, 4], [16, 16], mode=sum)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-traces", action = "store_true", default=False)
    ...
    with torch.autograd.profiler.profile(args.enable_profiling, use_gpu) as prof:
        while k < args.nepochs:
            ...
            for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
                ...
                if args.enable_memory_profiling and not args.qr_flag:
                    if not use_gpu:
                        for K in range(len(lS_o)):
                            print("TABLE {} [{} entries, cpu] <- READ {}".format(K, ln_emb[K], lS_i[K]))
                    else:
                        for K in range(len(lS_o)):
                            print("TABLE {} [{} entries, cuda: {}] <- READ {}".format(K, K % ndevices, ln_emb[K], lS_i[K]))

                elif args.enable_memory_profiling and args.qr_flag:
                    q_lS_i = []
                    r_lS_i = []
                    for K in range(len(lS_o)):
                        q_lS_i.append((lS_i[K] / args.qr_collisions).long())
                    #print(int(list(set(q_lS_i[0]))[0]))
                    for K in range(len(lS_o)):
                        r_lS_i.append((lS_i[K] % args.qr_collisions).long())
                    if not use_gpu:
                        for K in range(len(lS_o)):
                            print("TABLE {} [{} entries, cpu] <- READ {}".format(K, ln_emb[K], q_lS_i[K]))
                            print("TABLE {} [{} entries, cpu] <- READ {}".format(K, ln_emb[K], r_lS_i[K]))
                    else:
                        for K in range(len(lS_o)):
                            print("TABLE {} [{} entries, cuda: {}] <- READ {}".format(K, K % ndevices, ln_emb[K], q_lS_i[K]))
                            print("TABLE {} [{} entries, cuda: {}] <- READ {}".format(K, K % ndevices, ln_emb[K], r_lS_i[K]))


                if args.output_traces:
                    random_on = False
                    #print("idx {}, offset {}, indices {}".format(j, lS_o, lS_i))
                    #logical address = base address + idx * 4KB + offset (contiguous)
                    ...
                    for K in range(len(lS_o)):
                        ...
                        if random_on:
                            tmp_ch_arr = ""
                            for i in range(8):
                                rand_num = hex(random.randint(0, 15))
                                tmp_ch_arr = str(rand_num)[2:] + tmp_ch_arr
                            tmp_str = tmp_str + (8-len(tmp_ch_arr)) * "0" + tmp_ch_arr
                        else:
                            for idx in range(len(lS_i)):
                                if args.qr_flag:
                                    tmp_str = "0x"
                                    base_addr = K * (16 ** 5) # 0x000k 0000
                                    base_addr = base_addr + q_lS_i[K][idx]*1024
                                    tmp_str = tmp_str + (8-len(str(hex(base_addr))[2:]))*"0" +  str(hex(base_addr))[2:]
                                    trace_list.append("{} R\n".format(tmp_str))

                                    tmp_str = "0x"
                                    base_addr = K * (16 ** 5)
                                    base_addr = base_addr + r_lS_i[K][idx]*1024
                                    tmp_str = tmp_str + (8-len(str(hex(base_addr))[2:]))*"0" +  str(hex(base_addr))[2:]
                                    trace_list.append("{} R\n".format(tmp_str))
                                    ...
                                    tmp_str = "0x"
                                    base_addr = K * (16 ** 5)
                                    base_addr = base_addr + lS_i[K][idx]*1024 # page size = 1 KB
                                    tmp_str = tmp_str + (8-len(str(hex(base_addr))[2:]))*"0" +  str(hex(base_addr))[2:]
                                    trace_list.append("{} R\n".format(tmp_str))


                    #base_addr = base_addr + j*args.arch_sparse_feature_size*ln_emb[k] # embedding table size = feature size * number of data
                ...
                if (should_print or should_test) and not args.enable_memory_profiling and not args.output_traces:
    ...
    # profiling
    if args.output_traces:
        with open("../ramulator/dram.trace", "w") as f:
            for i in range(len(trace_list)):
                f.write(trace_list[i])
